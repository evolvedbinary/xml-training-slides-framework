...
<xsl:stylesheet 
  ...
  <xsl:param name="css-file-name" select="'expertml.css'"/>
  
  <xsl:variable name="quot"><![CDATA["]]></xsl:variable>
  <xsl:variable name="apos"><![CDATA[']]></xsl:variable>
  
  <xsl:output method="xhtml" html-version="5" indent="true"/>
  ...
  <xsl:template match="slide:course|slide:set">
    <html xmlns="http://www.w3.org/1999/xhtml">
      <head>
        ...
        <title>
          ...
          <xsl:sequence select="($titles, 'eXpertML Training (c) 2020')[1]"/>
        </title>
  ...
  <xsl:template match="slide:ref" mode="html">
    <xsl:try>
      <xsl:apply-templates select="doc(resolve-uri(@href, base-uri(.)))" mode="#current"/>
      <xsl:catch>
        <xsl:message expand-text="true">ERROR: Can't read {resolve-uri(@href, base-uri(.))}</xsl:message>
      </xsl:catch>
    </xsl:try>
  </xsl:template>
  ...
  <xsl:template name="slide:codeClass">
    <xsl:variable name="lineNumbers" as="xs:string?">
      <xsl:if test="not(slide:contains(@class, 'no-line-numbers'))">
  ...
  <xsl:template match="slide:code/@type" mode="code">
    <xsl:attribute name="class" select="slide:addToken('language-'||., ../@class)"/>
  </xsl:template>
  
  <xd:doc>
    <xd:desc>Add syntax highlighting for markup languages in code</xd:desc>
  </xd:doc>
  <xsl:template match="slide:code/@type[. = ('xml', 'xhtml')]" mode="code">
    <xsl:attribute name="class" select="slide:addToken('language-markup', ../@class)"/>
  </xsl:template>
  ...
  <xsl:function name="slide:addToken" as="xs:string">
    ...
    <xsl:value-of select="string-join(($token, tokenize($attribute, '\s')), ' ')"/>
  </xsl:function>
  ...
  <xsl:function name="slide:contains" as="xs:boolean">
    ...
    <xsl:value-of select="$tokenList!tokenize('\s') = $tokens"/>
  </xsl:function>
